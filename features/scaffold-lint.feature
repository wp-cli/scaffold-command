Feature: Lint scaffolded code
  ...
  Scenario: Scaffold plugin and lint it
    ...
    When I run `{PLUGIN_DIR}/test-plugin/vendor/bin/phpcs --standard={PLUGIN_DIR}/test-plugin/.phpcs.xml.dist {PLUGIN_DIR}/test-plugin/test-plugin.php`
    Then the return code should be 0

  Scenario: Scaffold post-type and lint it
    Given a WP install
    And I run `wp theme install twentytwentyone --activate`
    And I run `wp eval 'echo STYLESHEETPATH;'`
    And save STDOUT as {STYLESHEETPATH}
    And I run `wp plugin path`
    And save STDOUT as {PLUGIN_DIR}

    # Create a test plugin for installing phpcs
    When I run `wp scaffold plugin lint-tester --skip-tests`
    Then the return code should be 0
    ...
    When I run `{PLUGIN_DIR}/lint-tester/vendor/bin/phpcs --standard=WordPress {STYLESHEETPATH}/post-types/movie.php`
    Then the return code should be 0
  ...
  Scenario: Scaffold taxonomy and lint it
    ...
    When I run `{PLUGIN_DIR}/lint-tester/vendor/bin/phpcs --standard=WordPress {STYLESHEETPATH}/taxonomies/genre.php`
    Then the return code should be 0
  ...
  Scenario: Scaffold plugin tests and lint them
    ...
    When I run `{PLUGIN_DIR}/test-plugin/vendor/bin/phpcs --standard={PLUGIN_DIR}/test-plugin/.phpcs.xml.dist {PLUGIN_DIR}/test-plugin/tests/bootstrap.php {PLUGIN_DIR}/test-plugin/tests/test-sample.php`
    Then the return code should be 0
  ...
  Scenario: Scaffold child theme and lint it
    ...
    When I run `{PLUGIN_DIR}/lint-tester/vendor/bin/phpcs --standard=WordPress {THEME_DIR}/test-child/functions.php`
    Then the return code should be 0
  ...
  Scenario: Scaffold block and lint it
    ...
    Then STDOUT should not be empty
    And the {PLUGIN_DIR}/blocks/the-green-mile.php file should exist

    # Run phpcs on the scaffolded block
    When I run `{PLUGIN_DIR}/vendor/bin/phpcs --standard=WordPress {PLUGIN_DIR}/blocks/the-green-mile.php`
    Then the return code should be 0
